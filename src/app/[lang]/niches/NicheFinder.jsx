...
import { normalizeLang, supportedLang } from "../../i18n";

function getLocalizedCalcUrl(lang) {
    const safeLang = normalizeLang(lang);
    return `/${safeLang}/youtube/revenue-calculator`;
}
...
// Helper to extract lang from path ("/en/...", "/fr/...")
function useLangFromPath() {
    const pathname = usePathname?.() || ""; // in case usePathname is undefined
    // Expect path: "/[lang]/something..."
    const match = pathname.match(/^\/([a-zA-Z0-9_-]+)(\/|$)/);
    return match ? match[1] : "en";
}

// ── NicheCard ──────────────────────────────────────────────
function NicheCard({ niche, index, openId, setOpenId, lang }) {
    ...
    function goToCalculator() {
        ...
        router.push(`/${normalizeLang(lang)}/youtube/revenue-calculator${query}`);
    }
...
export default function NicheFinder() {
    ...
    const filtered = niches
        ...
        .sort((a, b) => {
            ...
            if (sort === "potential") return b.economic_potential - a.economic_potential;
            return 0;
        });

    // Track la recherche avec debounce
    useEffect(() => {
        if (!search) return;
    ...
    }, [search, filterFormat, filtered.length]);
    ...
    function handleFormatFilter(value) {
        ...
        if (typeof gtag === "function") {
            gtag("event", "niche_filter_format", { filter_value: value });
        }
    }
            ...
            <div className="nf-inner">
                ...
                {filtered.length > visibleCount && (
                    <div className="nf-showmore-wrap">
                        <button className="nf-showmore" onClick={handleShowMore}>
